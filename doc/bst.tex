...
By definition, each node in a binary searcg tree contains a left child and
...
This example is taken from Corman et al.~\cite[p. 74]{cormen:th:1990}
Problem 4-6.

\begin{align}
F(z) & = \sum_{i=0}^{\infty} F_ix^i\\
\label{eqn:fibonacci_opsgf}
     & = 0 + z + x^2 + 2z^3 + 3z^4 + 5z^5 + 8z^6 + 13z^7 + 21z^8+\cdots
\end{align}
...
zF(z) = z^2 + x^3 + 2z^4 + 3z^5 + 5z^6 + 8z^7 + 13z^8 + 21z^9+\cdots
\end{equation}
...
F(z) - zF(z) & = z + z^3 + z^4 + 2z^5 + 3z^6 +\cdots\\
        F(z) & = z + zF(z) + z^2(0 + z + z^2 + 2Z^3 + 3z^4 +\cdots)\\
             & = z + zF(z) + z^zF(z).
\end{align}
